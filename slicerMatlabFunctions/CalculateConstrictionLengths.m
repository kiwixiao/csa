...
for j = 1:20
    ...
    elseif size(constrictedCoords,1) > 1
       gaps = diff(constrictedCoords);
        ...
        if isempty(first) || isempty(last)
            lengthOfConstriction(j) = mean(diff(arcLength));
        else
    ...
    elseif size(constrictedCoords,1) == 1
        ...
        if constrictedCoords < length(arcLength)
            lengthOfConstriction(j) = ((arcLength(constrictedCoords+1) - arcLength(constrictedCoords))/2) / arcLength(end);
        elseif constrictedCoords > 1
            lengthOfConstriction(j) = ((arcLength(constrictedCoords) - arcLength(constrictedCoords-1))/2) / arcLength(end);
        else
            lengthOfConstriction(j) = mean(diff(arcLength));
        end
    else
